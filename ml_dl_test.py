...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Ejemplos de uso:
  python ml_dl_test.py
  python ml_dl_test.py --input datos_spoofing.csv
  python ml_dl_test.py -i gnss_log_2025_11_22_17_18_54_spoofed_features.csv
        """
    )
    ...
    parser.add_argument(
        '-i', '--input',
        type=str,
        default='gnss_log_2025_11_22_17_18_54_spoofed_features.csv',
        help='Ruta al archivo CSV con los datos de entrada (default: gnss_log_2025_11_22_17_18_54_spoofed_features.csv)'
    )
...
def model_training_evaluation():
    ...
    # Comentar o eliminar estas líneas que no usas:
    """
    # Valorar si son necesarias estas dos líneas
    rus = RandomUnderSampler(random_state=42)
    X_rus, y_rus = apply_balancing(X, y, rus, "Random Undersampling")
    """
    ...
    y_pred = knn_grid.predict(X_test)  # Cambiar X_rus_test por X_test

    # Calculating metrics

    knn_grid_score = accuracy_score(y_test, y_pred)  # Cambiar y_rus_test por y_test
    ...
    try:
        ...
        labels = None
    plot_confusion_matrix_percent(cm, labels=labels, filename="knn.png")
    ...
    y_pred = gb_grid.predict(X_test)  # Cambiar X_rus_test por X_test

    # Calculating metrics

    gb_grid_score = accuracy_score(y_test, y_pred)  # Cambiar y_rus_test por y_test
    ...
    try:
        ...
        labels = None
    plot_confusion_matrix_percent(cm, labels=labels, filename="gaussian_nb.png")
    ...
    xgb_grid_score = accuracy_score(y_test, y_pred)  # Cambiar y_rus_test por y_test
    ...
    try:
        ...
        labels = None
    plot_confusion_matrix_percent(cm, labels=labels, filename="xgboost.png")
    ...
    # Use the existing train/test splits for the deep learning model
    ...
    # Use explicit labels from y_test unique values
    try:
        labels = [str(x) for x in np.unique(np.concatenate((y_test, d_pred)))]
    except Exception:
        labels = None
    plot_confusion_matrix_percent(cm, labels=labels, filename="deep_learning.png")

    # Añadir la accuracy del modelo de deep learning a la tabla de resultados y graficar
    dl_test_score = accuracy
    dl_val_score = max(history.history['val_accuracy']) * 100  # Mejor accuracy de validación (%) durante el entrenamiento
    print(f"Mejor accuracy de validación (Deep Learning): {dl_val_score:.2f}%")
    print(f"Accuracy en test (Deep Learning): {dl_test_score*100:.2f}%")
